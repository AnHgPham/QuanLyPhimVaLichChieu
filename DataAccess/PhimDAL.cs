...
namespace QuanLyPhimVaLichChieu.DataAccess
{
    public class PhimDAL
    {
        ...
        public Phim? GetById(int maPhim)
        {
            ...
            DataTable dt = DatabaseHelper.ExecuteQuery(query, new SqlParameter("@MaPhim", maPhim));
            if (dt.Rows.Count > 0)
        ...
        public int Insert(Phim p)
        {
            ...
            var result = DatabaseHelper.ExecuteScalar(query,
                new SqlParameter("@TenPhim", p.TenPhim),
                new SqlParameter("@MaTheLoai", p.MaTheLoai),
                new SqlParameter("@ThoiLuong", p.ThoiLuong),
                new SqlParameter("@QuocGia", (object?)p.QuocGia ?? DBNull.Value),
                new SqlParameter("@MoTa", (object?)p.MoTa ?? DBNull.Value),
        ...
        public bool Update(Phim p)
        {
            string query = @"UPDATE Phim SET 
                             ...
                             QuocGia = @QuocGia, MoTa = @MoTa, TrangThai = @TrangThai, NgayCapNhat = GETDATE()
                             WHERE MaPhim = @MaPhim";
            int rows = DatabaseHelper.ExecuteNonQuery(query,
                new SqlParameter("@MaPhim", p.MaPhim),
                new SqlParameter("@TenPhim", p.TenPhim),
                new SqlParameter("@MaTheLoai", p.MaTheLoai),
                new SqlParameter("@ThoiLuong", p.ThoiLuong),
                new SqlParameter("@QuocGia", (object?)p.QuocGia ?? DBNull.Value),
        ...
        public bool Delete(int maPhim)
        {
            ...
            int rows = DatabaseHelper.ExecuteNonQuery(query, new SqlParameter("@MaPhim", maPhim));
            return rows > 0;
        }
